...
import { PROJECTS, SCHOOL_PROJECTS } from "../constants";
...
const Projects = () => {
  ...
  return (
    <div id="projets" className="py-20 text-white max-w-6xl mx-auto px-4">
      ...
      {/* === PROJETS SCOLAIRES === */}
      <h2 className="text-3xl font-bold my-16 text-center uppercase tracking-widest text-blue-400">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
        {SCHOOL_PROJECTS.map((project, index) => (
          <div
            key={index}
            onClick={() => setSelectedProject(project)}
            className="flex flex-col justify-between h-[540px] bg-gradient-to-br from-[#111] to-[#1c1c1c] p-6 rounded-2xl shadow-xl border border-white/10 hover:shadow-blue-500/20 transition-all duration-300 cursor-pointer"
          >
            <div className="text-center">
              ...
              >
                {project.images.map((img, i) => (
                  <SwiperSlide key={i}>
                    <img
                      src={img}
                      alt={`school-project-${index}-slide-${i}`}
                      className="h-48 w-full object-contain bg-black rounded"
                    />
                  </SwiperSlide>
                ))}
            ...
            <div className="mt-6 flex justify-center gap-4">
              ...
              <a
                ...
                rel="noopener noreferrer"
                onClick={(e) => e.stopPropagation()}
                className="inline-flex items-center gap-2 bg-blue-600 hover:bg-blue-500 text-white font-semibold px-4 py-2 rounded-full text-sm transition"
              >
                <HiArrowUpRight className="w-4 h-4" /> Démo
              </a>
            </div>
